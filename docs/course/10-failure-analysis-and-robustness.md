...
- **条件成功率**：弱区判断必须用该区成功数/总数，而不是只数失败；
- **探索与确认**：同一数据用于发现弱区后，要用新固定分布确认；
- **互斥完备分类**：每个失败只进入一个类别，类别数之和等于总失败数；
- **轨迹里程碑**：approach → reached → contact → lift → success 定位断点；
- **机制假设**：下一项改动针对 dominant failure，并写出可否定预测；
- **回归分布**：改弱区时仍需保护 original 和 hard-bin 表现。
