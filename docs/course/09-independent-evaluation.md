...
- **冻结策略评估**：只做 inference，不更新参数，也不继续挑 checkpoint；
- **点估计**：`successes/episodes` 描述本批采样观察到的比例；
- **Wilson 区间**：描述二项抽样不确定性，不替代实际点估计；
- **固定门槛**：运行前定义的 PASS/FAIL 规则，不能看完结果后修改；
- **aggregate 与 worst seed**：同时看总体表现和最弱随机批次；
- **随机样本与精选回放**：前者能估计率，后者只能帮助理解行为。
