...
- **基线复现**：不改变实验语义，验证官方方案能否重现；
- **工程适配**：为硬件或运行条件调整实现，并明确降低了什么主张；
- **新实验**：提出新的假设或改动，必须与基线分支和证据分开；
- **复现契约**：运行前固定代码、配置、checkpoint、种子、门槛和产物；
- **证据链**：任何结论都能回到一个 JSON、一次运行和一个 Git commit。
