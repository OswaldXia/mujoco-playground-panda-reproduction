...
- **MDP**：用状态、动作、转移、奖励和折扣描述智能体与环境的闭环；
- **reward 与 return**：前者是当前一步的分数，后者是从当前开始的折扣累计；
- **value 与 advantage**：前者估计未来回报，后者判断动作相对基线好多少；
- **GAE**：用 `gamma` 和 `lambda` 混合多步 TD residual，估计 advantage；
- **概率比与 PPO clipping**：限制新策略对旧数据中动作概率的过大改变；
- **on-policy**：更新使用当前/近当前策略收集的数据，不能无限复用旧轨迹。
